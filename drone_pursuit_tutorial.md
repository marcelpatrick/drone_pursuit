...
> > 💻 **NO HARDWARE — simulation only.** The drone stays in its box. This block needs the three measurements from 🔌 subchapter 1.4 in `project_notes.txt`. **If you do not have the drone yet**, create the file with the estimates below, marked as estimates, and replace them after 1.4. Every number in 3.1 is built from this file, and the 3.1 checker reads it.
...
Do this so the 3.1 checker can refer to a project_notes.txt file and pass - even if this file still contains dummies.

```
ESTIMATES - replace with real Tello measurements from 1.4
control rate        : 20 Hz            → decimation = 5 in the cfg
video delay         : 175 ms mean, 99–219 ms range
                      → 2–5 control steps at 20 Hz
telemetry available : speeds (cm/s), attitude angles. NO angular rates.
...
The 3.1 checker will pass with a `[??]` warning that these are the tutorial's example numbers. That warning is intended: it stays until you replace them with measurements.
...
# ── SECTION: class QuadcopterEnv, method __init__ ───────────────────────────
        ...
        self._robot_weight = (self._robot_mass * self._gravity_magnitude).item()   # ← EXISTING
...
Expect the print to show roughly `3e-05`. Mass tells you how hard the drone is to *push*; inertia tells you how hard it is to *turn*.

**Edit 2 of 3 — replace the force outputs with stick commands, in `_pre_physics_step`:**
...
# ── SECTION: class QuadcopterEnv, method _pre_physics_step ──────────────────
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        # cancel gravity along the drone's own axes, so "up" stays up even when tilted
        force_b -= self._robot_weight * self._robot.data.projected_gravity_b

        # set_external_force_and_torque reads forces in the drone's OWN frame — no rotation needed
        self._thrust[:, 0, :] = force_b
        yaw_error = desired_yaw_rate - self._robot.data.root_ang_vel_b[:, 2]
        self._moment[:, 0, 2] = self.cfg.yaw_gain * yaw_error * self._robot_izz
        # ▲▲▲ END OF INSERT ▲▲▲
```

**Edit 3 of 3 — the four constants it reads, in the config class:**
...
**Why the force is not rotated.** Isaac Lab applies external forces in the drone's own frame by default. Rotating the force into world directions first turns it twice, so once the drone has turned, "forward" pushes sideways and drones drift out of their allowed height band. The gravity term uses `projected_gravity_b` (which way "down" is, seen from the drone) so it keeps cancelling gravity when the drone tilts.
...
**Automatic version (recommended).** `check_3_1.py` runs 64 drones for 300 steps, records every camera reading before and after the delay buffer, and checks all four checkpoint items, naming the tutorial Part to fix for any failure. 

Copy `check_3_1.py`
Save it as `C:\projects\drone_pursuit\drone_pursuit\scripts\checker_code\check_3_1.py`, then run:


<details><summay>Expand check_3_1.py</summay>
...
```bat
conda activate env_drone
python C:\projects\drone_pursuit\drone_pursuit\scripts\checker_code\check_3_1.py
```

Expected result: `RESULT: Checkpoint 3.1 passed`, with one `[??]` under item 4 while `project_notes.txt` still holds estimates. Run it before adding 3.3's reading noise, which makes held values wiggle on purpose.

</details>

> ✅ **Checkpoint 3.1** — run `check_3_1.py`. The last line must read `RESULT: Checkpoint 3.1 passed`.
>
> 1. **The env runs cleanly:** the console prints `defender yaw inertia` around `3e-05` (not `0.00e+00`). The report shows `observation_space = 17`, `decimation = 5 -> 20.0 Hz control` matching your notes, all 300 steps × 64 envs with no errors or `NaN`, and slot order `speed 0-2, tilt 3-5, camera 6-12, commands 13-16`.
